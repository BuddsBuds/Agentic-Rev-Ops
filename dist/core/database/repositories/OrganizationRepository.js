...
class OrganizationRepository extends BaseRepository_1.BaseRepository {
    constructor(db) {
        super(db, {
            ...
            schema: 'core',
            entityClass: Organization_1.Organization
        });
    }
    async findByDomain(domain) {
        return this.findOne({ where: { domain } });
    }
    async findBySubscriptionTier(tier) {
        return this.find({
            where: { subscription_tier: tier },
            orderBy: 'created_at DESC'
        });
    }
    async findHighUsageOrganizations(threshold = 0.8) {
        const query = `
      SELECT 
        o.*,
    ...
    async updateSubscriptionTier(organizationId, newTier) {
        const org = await this.findById(organizationId);
        if (!org)
            return null;
        org.set('subscription_tier', newTier);
    ...
    async getOrganizationsRequiringAttention() {
        const query = `
      SELECT 
        o.*,
        CASE 
